...
public class MaximaCalculator {
    ...
    private Map<Integer, ConsumedMaximaBuckets> consumedMaximaBucketsMap;

    private  Integer tariffId;

    private DateTime originalEntryTime;

    private DateTime originalExitTime;
...
//        logger.info("Month Max: " + defaultMonthMax);
    ...
    public MaximaCalculator(DateTime inTime,
                            ...
                            DateTime parkingSessionEntryTime,
                            DateTime parkingSessionExitTime,
                            Map<Integer,ConsumedMaximaBuckets> consumedMaximaBucketsMap,
                            DateTime originalEntryTime,
                            DateTime originalExitTime,
                            Integer tariffId) {
        this.inTime = inTime;
        this.outTime = outTime;
        this.consumedMaximaBucketsMap=consumedMaximaBucketsMap;
...
//        logger.info("Month Max: " + defaultMonthMax);
    ...
    public int calculate() {
        ...
        int listSize;
        int adjustedAmount;
        // check if Month then Week then Day max is achieved priority is Month>Week>Day
        //once we get what maxima is achieved then if the current out time is greater than
...
//        logger.info("dayTariffList size: " + dayTariffList.size());
    ...
    private int getFirstDayAmount() {
        ArrayList<HashMap<Integer, TariffSample>> tariffSampleFromMaximaBucket=checkForConsumedMaximaBuckets(tariffId,inTime,originalEntryTime);
    ...
    private ArrayList<HashMap<Integer, TariffSample>> checkForConsumedMaximaBuckets(Integer tariffId, DateTime inTime, DateTime originalEntryTime) {
        ArrayList<HashMap<Integer, TariffSample>> consumedMaximaList= new ArrayList<>();
        if(Objects.nonNull(consumedMaximaBucketsMap.get(tariffId))){
            Days days= Days.daysBetween(originalEntryTime,inTime);
            Weeks weeks= Weeks.weeksBetween(originalEntryTime,inTime);
            Months months= Months.monthsBetween(originalEntryTime,inTime);


            int dayNumber = Math.max(days.getDays(), 0);
            int weekNumber = Math.max(weeks.getWeeks(), 0);
            int monthNumber = Math.max(months.getMonths(), 0);

            if(consumedMaximaBucketsMap.containsKey(monthNumber)){
                HashMap<Integer,TariffSample> monthMaximaBucket= new HashMap<>();
                monthMaximaBucket.put(monthNumber,consumedMaximaBucketsMap.get(tariffId).getConsumedMonthMaximaBucket().get(monthNumber));
                 consumedMaximaList.add(monthMaximaBucket);
            }
            if(consumedMaximaBucketsMap.containsKey(weekNumber)){
                HashMap<Integer,TariffSample> weekMaximaBucket= new HashMap<>();
                weekMaximaBucket.put(weekNumber,consumedMaximaBucketsMap.get(tariffId).getConsumedMonthMaximaBucket().get(weekNumber));
                consumedMaximaList.add(weekMaximaBucket);
            }
            if(consumedMaximaBucketsMap.containsKey(dayNumber)){
                HashMap<Integer,TariffSample> dayMaximaBucket= new HashMap<>();
                dayMaximaBucket.put(monthNumber,consumedMaximaBucketsMap.get(tariffId).getConsumedMonthMaximaBucket().get(monthNumber));
                consumedMaximaList.add(dayMaximaBucket);
            }
        }
        return consumedMaximaList;
    }


    private ConsumedMaximaBuckets createConsumedMaximaBuckets(Integer amount,DateTime inTime, DateTime originalEntryTime, DateTime originalExitTime, TariffSample ts) {
        ConsumedMaximaBuckets consumedMaximaBuckets= new ConsumedMaximaBuckets();
        HashMap <Integer,MaximaCalculator.TariffSample> consumedDayMaximaBucket= new HashMap<>();
        HashMap<Integer,MaximaCalculator.TariffSample> consumedWeekMaximaBucket= new HashMap<>();
        HashMap<Integer,MaximaCalculator.TariffSample> consumedMonthMaximaBucket= new HashMap<>();


        Days days= Days.daysBetween(originalEntryTime,inTime);
        Weeks weeks= Weeks.weeksBetween(originalEntryTime,inTime);
        Months months= Months.monthsBetween(originalEntryTime,inTime);
        consumedMaximaBuckets.setOriginalInTime(originalEntryTime);
        consumedMaximaBuckets.setOriginalOutTime(originalExitTime);
        ...
        switch (ts.getType()){
            case DAY: consumedDayMaximaBucket.put(dayNumber,ts);
            break;
            case WEEK: consumedWeekMaximaBucket.put(weekNumber,ts);
            break;
            case MONTH: consumedMonthMaximaBucket.put(monthNumber,ts);
            break;
        }

        consumedMaximaBuckets.setConsumedDayMaximaBucket(consumedDayMaximaBucket);
        consumedMaximaBuckets.setConsumedWeekMaximaBucket(consumedWeekMaximaBucket);
        consumedMaximaBuckets.setConsumedMonthMaximaBucket(consumedMonthMaximaBucket);
        return consumedMaximaBuckets;
    }
...
//            logger.info("Merged: {}", t.toString());
    ...
    private void generateFinalList() {
        ...
        // add the final list to consumed Map for that TariffId
    }
